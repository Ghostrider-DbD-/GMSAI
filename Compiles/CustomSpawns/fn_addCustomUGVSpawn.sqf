...
params[
	...
	"_UGVs"				// weighted array with one or more classNames ["Name1",1,"Name2",2, ... etc]
];
[format["<START> addCustomUGVSpawn: marker pos %1 | shape %2 | size %3",_pos,_shape,_size]] call GMSAI_fnc_log;
private "_m";
if (GMSAI_debug >= 0) then 
{
	_m = createMarker[format["customUGV%1",random(1000000)],_pos];	
	...
	_m = createMarkerLocal [format["customUGV%1",random(1000000)],_pos];
	_m setMarkerShapeLocal _shape;
	_m setMarkerPosLocal _pos;
	_m setMarkerSizeLocal _size;
};
_types = [
	[GMSAI_ugv,_groups,_UGVs]
];
...
[format["addCustomUGVSpawn: marker pos %1 | shape %2 | size %3",_pos,_shape,_size]] call GMSAI_fnc_log;